...
export const actions = {
	default: async ({ request }) => {
		...
		const emailText = data.get('emailText') as string;
		...
		//check for receiver markers in email text
		if (!emailText.includes('@receiver')) {
			return fail(422, { missingMarkers: true });
		}
...
async function sendEmail(toEmail: string, emailText: string) {
	...
	try {
		...
		const info = await transporter.sendMail({
			from: sender,
			to: toEmail,
			subject: "It's time to surprise a friend!",
			text: emailText
			// html: "", // html body
		});
